...
int main()
{
    ...
    if (command=='S')
    {
        ...
        letter=highestProb(possibleWords,guessed);
     //   cout<<letter<<possibleWords.size()<<"\n";
        while (cin>>command)
        {
            guessed[letter-'a']=1;
            cin>>l>>word>>chances>>numguessed;
            cout<<l<<word<<chances<<numguessed;
            forall(i,0,numguessed)
                cin>>tempc;
            if (word.find(letter)!=string::npos)
                update(possibleWords,word,letter,true);
            else
                update(possibleWords,word,letter,false);
            letter=highestProb(possibleWords,guessed);
            cout<<"\n";
        }
    }
